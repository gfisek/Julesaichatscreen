...
import { MiniJules, type IntroPhase } from "./components/MiniJules";
...
export default function App() {
  ...
  useLayoutEffect(() => {
    ...
    if (fromIntroRef.current) {
      // Intro'dan geliyorsa: sadece fade-in, translate yok
      ...
      const id = requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          ...
          boxRef.current.style.transition = "opacity 800ms cubic-bezier(0.16,1,0.3,1)";
  ...
  const runIntroSequence = useCallback(() => {
    ...
    // Hedef: dikey merkez - 80px yukarısı
    const targetY = viewH / 2 - elH / 2 - 80;

    // Pinned-right için yatay kaydırma hesabı
    const isPinned = isPinnedRight && !isMobile;
    const targetX  = isPinned ? (viewW / 2 - (viewW - 195)) : 0;

    // Faz 1: Merkeze uçuş — 1400ms spring (hafif aşan bezier)
    requestAnimationFrame(() => {
      // Yumuşak ease-out — overshoot/bounce yok
      el.style.transition = `transform 1400ms cubic-bezier(0.22, 1, 0.36, 1)`;
      el.style.transform  = isPinned
    ...
    // Faz 2: 1400ms sonra iki-parçalı typewriter başlat
    addTimer(() => {
      setIntroPhase("typing");

      // Yardımcı: bir stringi karakter karakter yazar
      const typeStr = (str: string, speedMs: number, onDone: () => void) => {
        let i = 0;
        const step = () => {
          setIntroText(str.slice(0, i));
          i++;
          if (i <= str.length) addTimer(step, speedMs);
          else onDone();
        };
        step();
      };

      // Çıkış animasyonu — ortak fonksiyon
      const startExit = () => {
        setIntroPhase("exit");
        // PART2 metni ekranda kalsın — element aşağı inerken hâlâ görünür

        // Faz 3: ease-in gravity; opacity 1150ms gecikmeli → ~%75'te solar
        el.style.transition = `transform 1700ms cubic-bezier(0.4, 0, 1, 1), opacity 400ms ease-in 1150ms`;
        el.style.transform  = `translateY(${viewH + 100}px)`;
        el.style.opacity    = "0";

        // Jules 1050ms'de fade-in başlar
        addTimer(() => {
          fromIntroRef.current = true;
          setIsMinimized(false);
        }, 1050);

        // MiniJules DOM'dan çıkar
        addTimer(() => {
          setIntroPhase("done");
      ...
      // Sekans: PART1 yaz → 900ms bekle →
      //         PART1'i silme, ikinci satıra PART2 yaz → 1300ms bekle → direkt çık
      typeStr(INTRO_PART1, 30, () => {
        addTimer(() => {
          // PART1 ekranda kalıyor — PART2 yeni satırda yazılıyor
          let idx = 0;
          const typePart2 = () => {
            setIntroText(INTRO_PART1 + (idx > 0 ? "\n" + INTRO_PART2.slice(0, idx) : ""));
            idx++;
            if (idx <= INTRO_PART2.length) addTimer(typePart2, 30);
            else addTimer(startExit, 1300);
          };
          typePart2();
        }, 900);
      });
