...
(function (global) {
  ...
  JulesWidget.prototype._createDOM = function () {
    ...
    this._rebuildPanel();
    // (open state applied by caller via .open())
  };
  ...
  JulesWidget.prototype._buildEmojiContent = function () {
    ...
    if (EMOJIS[st.emojiIndex] === 'BOT_ICON') {
      ...
      div.querySelector('svg').style.color = 'white';
      return div;
    } else {
  ...
  JulesWidget.prototype._refreshPanelSwitch = function () {
    ...
    // update track bg + thumb position
    ...
    // track.children (direct): [0]=leftLines, [1]=rightLines, [2]=thumb
  ...
  JulesWidget.prototype._applyTheme = function () {
    // Full rebuild: all DOM recreated with new theme tokens
    this._rebuildPanel();
    this._applyOpenState();
  };

  JulesWidget.prototype._updateFavBtn = function () {
    // Rebuild header to reflect new fav count
    this._rebuildHeader();
  };
  ...
  JulesWidget.prototype._theme = function () {
    ...
    return {
      bg:             'transparent',
      border:         d ? '#1d3547' : '#e5e7eb',
      textPrimary:    d ? '#cfe8f4' : '#111827',
      textSecondary:  d ? '#6fa8bf' : '#6b7280',
      textMuted:      d ? '#3d7089' : '#9ca3af',
      userBubble:     d ? '#1a3247' : '#F2F2F3',
      userBubbleText: d ? '#cfe8f4' : '#374151',
      inputBg:        d ? '#132230' : '#f9fafb',
      accentColor:    d ? '#4dc4ce' : '#0a6e82',
      accentDimBg:    d ? 'rgba(77,196,206,0.10)' : '#e6f7f9',
  ...
  JulesWidget.prototype._countFavs = function () {
    var st = this._st;
    return st.panelSessions.reduce(function (n, s) {
      return n + s.cards.filter(function (c) { return st.likedCards.has(s.id + '-' + c.id); }).length;
    }, 0);
  };
